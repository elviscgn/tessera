...
### Engine-track implementation (M16–M23)

The continuous arena track is implemented in `rust/crates/tessera-arena`. `Fixed` is a signed 64-bit fixed-point with 10 fractional bits (micro-relative units when scaled by 1024); arithmetic is integer-only, saturating where mandated, and `sqrt` is an integer binary search. `ArenaLayout` validates dimensions at construction, and `ArenaSimulation` owns the authoritative match: a 20 Hz tick schedule, phases (`Setup`/`Aiming`/`Releasing`/`Resolved`), turns, deterministic narrow-pair contact with equal-mass restitution and positional separation, wall bounds with restitution and tangential damping, west/east pocket goals, rest thresholds and a hard per-leg tick cap, power handles (the `POWER_DOUBLE_SHOT`), and automatic possession alternation. Every command the simulation accepts is persisted as a versioned replay record; `ArenaSimulation::replay` rebuilds a byte-identical match, and the canonical state hash (BLAKE3 over a versioned encoding of layout, tick, phase, score, bodies, and pending commands) is the native/Wasm parity checkpoint.

The Wasm adapter (`ArenaWasm`) owns one authoritative instance per session; commands cross as semantic JSON records parsed into the same `ArenaCommand` values the native crate encodes (`submit_commands_json`), so the binary encoding lives only in Rust, and event/snapshots serialize from Rust state. `ArenaSession` (TypeScript) sequences applied commands and drives stepping/resolution over that core; `src/presentation/arena` provides pure interpolation between authoritative snapshots, the arena camera projection, and event-to-presentation mapping, none of which enter the state hash. `tessera-cli arena play` is the headless vertical slice and self-verifies its match against its own replay log. `examples/arena-external-consumer` is the outside-workspace proof that the crate composes from its public API.
