...
The Wasm adapter (`ArenaWasm`) owns one authoritative instance per session; commands cross as the same deterministic byte encoding as the native crate, and event/snapshots serialize from Rust state. `ArenaSession` (TypeScript) sequences applied commands and drives stepping/resolution over that core; `src/presentation/arena` provides pure interpolation between authoritative snapshots, the arena camera projection, and event-to-presentation mapping, none of which enter the state hash. `tessera-cli arena play` is the headless vertical slice and self-verifies its match against its own replay log. `examples/arena-external-consumer` is the outside-workspace proof that the crate composes from its public API.
