...
- For continuous arena work, keep positions, velocities, impulses, collision outcomes, goals, match phases, scores, and turn deadlines in Rust-owned fixed-point stores. The browser may preview an aim vector and interpolate snapshots, but it must never resolve motion or scoring.
- Submit semantic arena intents such as formation, ready, aim-and-release, or power activation. Do not record pointer samples, animation frames, or renderer contacts as simulation commands.
- Keep arena geometry narrow and explicit. Stable contact ordering, checked arithmetic, deterministic substeps, rest thresholds, and bounded turn duration are required before adding new collision shapes or a physics dependency.
...
The current implementation scope includes the Babylon lifecycle, the Worker readiness bridge, the orthographic camera model, named camera actions, coordinate conversion, authoritative grid occupancy, declarative object registration, Rust-backed placement queries and commands, the debug-grid projection, slot/generation selection, deterministic waits, diagnostics, Rust-owned save/load and replay metadata, browser persistence adapters, the development test bridge and annotated captures, versioned reproduction manifests, and their tests. Continuous arena gameplay, network transport, and consumer-owned Rust extensions belong to the separately gated engine track; React remains limited to development tooling and Scenario Lab.
